...
function App() {
  ...
  const copyToClipboard = useCallback(() => {
    ...
    const regularStudents = present
      .filter((roll) => roll.startsWith("22L31A05"))
      .sort()
      .map((roll) => roll.slice(-3).replace(/O/g, "Ο"));
    
    const lateralStudents = present
      .filter((roll) => roll.startsWith("23L35A0"))
      .sort()
      .map((roll) => roll.slice(-3).replace(/O/g, "Ο"));
    
    const readmittedStudents = present
      .filter((roll) => roll.startsWith("21L31A05"))
      .sort()
      .map((roll) => roll.slice(-3).replace(/O/g, "Ο"));
    
    // Combine in the desired order: Regular → Lateral → Re-admitted
    const orderedStudents = [...regularStudents, ...lateralStudents, ...readmittedStudents];
    const text = orderedStudents.join(", ");
  ...
  return (
    ...
    >
      ...
      <h1 className="mb-4 text-[18px]">Re-admitted</h1>
      <div
        style={{
          display: "grid",
          gridTemplateColumns:
      ...
      >
        {readmittedRollNumbers.map((roll) => (
          <div
            key={roll}
            style={{
              backgroundColor: present.includes(roll)
      ...
      {present.length > 0 && (
        ...
        >
          ...
          {/* Re-admitted Students */}
          {present.filter((roll) => roll.startsWith("21L31A05")).length > 0 && (
            <div style={{ marginBottom: 20 }}>
              <h4
                style={{ color: "#333", marginBottom: 10, fontSize: "1.1rem" }}
              >
                Re-admitted (
                {present.filter((roll) => roll.startsWith("21L31A05")).length})
              </h4>
              <textarea
                value={present
                  .filter((roll) => roll.startsWith("21L31A05"))
          ...
          >
            ...
            >
              ...
              >
                ...
                {markingMode === 'present' ? present.filter(roll => !roll.startsWith("21L31A05")).length : (rollNumbers.length + lateralRollNumbers.length) - present.filter(roll => !roll.startsWith("21L31A05")).length}
              </span>
              <span
                style={{
                  color: "#f44336",
              ...
              >
                ...
                {markingMode === 'absent' ? present.filter(roll => !roll.startsWith("21L31A05")).length : (rollNumbers.length + lateralRollNumbers.length) - present.filter(roll => !roll.startsWith("21L31A05")).length}
              </span>
            </div>
          </div>
        </div>
