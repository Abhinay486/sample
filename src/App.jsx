...
function App() {
  ...
  const copyToClipboard = useCallback(() => {
    const text = present.sort().join(", ");
    navigator.clipboard.writeText(text);
    toast.success(`${present.length} ${markingMode === 'present' ? 'presenties' : 'absenties'} copied to clipboard!`);
  }, [present, markingMode]);
    ...
    >
      ...
      <div>
        ...
        <div style={{ 
          ...
          <p style={{ 
            ...
            Currently marking selected students as: <strong>{markingMode}</strong>
          </p>
        </div>
            ...
            <div
              ...
              style={{
                backgroundColor: present.includes(roll) 
                  ? (markingMode === 'present' ? "#e8f5e8" : "#fde8e8") 
                  : "#fff",
                border: present.includes(roll)
                  ? (markingMode === 'present' ? "2px solid #4CAF50" : "2px solid #f44336")
                  : "2px solid #ddd",
                borderRadius: "12px",
                padding: "12px",
                cursor: "pointer",
              ...
              >
                ...
                <input
                  ...
                  style={{
                    marginLeft: 8,
                    transform: "scale(1.2)",
                    accentColor: markingMode === 'present' ? "#4CAF50" : "#f44336",
                    pointerEvents: "none",
                  }}
                />
              </div>
      ...
      {present.length > 0 && (
        ...
        >
          ...
          >
            <span>
              {markingMode === 'present' ? 'Presenties' : 'Absenties'} ({present.length})
            </span>
            <button
            ...
            >
              <Copy className="text-2xl" size={16} />
          ...
          {present.filter(roll => roll.startsWith('22L31A05')).length > 0 && (
            <div style={{ marginBottom: 20 }}>
              <h4 style={{ color: "#333", marginBottom: 10, fontSize: "1.1rem" }}>
                Regular ({present.filter(roll => roll.startsWith('22L31A05')).length})
              </h4>
              <textarea
                value={present.filter(roll => roll.startsWith('22L31A05')).sort().join(", ")}
                rows={3}
                style={{
                  width: "100%",
                  padding: "15px",
          ...
          {present.filter(roll => roll.startsWith('23L35A0')).length > 0 && (
            <div style={{ marginBottom: 20 }}>
              <h4 style={{ color: "#333", marginBottom: 10, fontSize: "1.1rem" }}>
                Lateral Entries ({present.filter(roll => roll.startsWith('23L35A0')).length})
              </h4>
              <textarea
                value={present.filter(roll => roll.startsWith('23L35A0')).sort().join(", ")}
                rows={3}
                style={{
                  width: "100%",
                  padding: "15px",
          ...
          >
            {markingMode === 'present' ? 'Present' : 'Absent'} students list - Click "Copy All" to copy complete data
          </p>
        </div>
      )}
